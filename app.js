...
// ─────────────────────────────────────────────────────────────────────
// HEALTH CHECK ENDPOINTS (for testing deployment)
// ─────────────────────────────────────────────────────────────────────

// Database Health Check
app.get('/api/health/database', async (req, res) => {
    try {
        const result = await require('./src/database').query('SELECT NOW() as current_time, version()');
        res.json({
            status: 'healthy',
...
// Frontend Health Check (simple page)
...
// Database Verification Page (HTML)
app.get('/health/database-page', async (req, res) => {
    try {
        const result = await require('./src/database').query('SELECT NOW() as current_time');
        res.send(`
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                ...
                <div class="details">
                    ...
                    <p><strong>Port:</strong> ${process.env.DB_PORT}</p>
                    <p><strong>Database:</strong> ${process.env.DB_NAME}</p>
                    <p><strong>User:</strong> ${process.env.DB_USER}</p>
                </div>
            </body>
            </html>
        `);
        ...
        res.status(503).send(`
            ...
            <head>
                ...
                <style>
                    body { font-family: Arial, sans-serif; margin: 50px; text-align: center; }
                    .error { color: red; }
                    .status-box { border: 2px solid red; padding: 20px; border-radius: 8px; display: inline-block; }
                </style>
            </head>
            <body>
                <div class="status-box">
                    ...
                    <p><strong>Check your database credentials and ensure PostgreSQL is running</strong></p>
                </div>
            </body>
            </html>
        `);
